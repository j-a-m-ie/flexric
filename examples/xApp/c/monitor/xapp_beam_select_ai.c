...
/* Bandit model: per-beam learned value and sample count */
static double g_q[MAX_BEAMS] = {0.0};
static uint64_t g_n[MAX_BEAMS] = {0};
...
static double env_double(const char* name, double def_val)
{
  const char* s = getenv(name);
  if (s == NULL || *s == '\0')
    return def_val;
...
static beam_policy_t env_policy(const char* name, beam_policy_t def_val)
{
  const char* s = getenv(name);
  if (s == NULL || *s == '\0')
    return def_val;

  if (strcmp(s, "bandit") == 0 || strcmp(s, "ml") == 0 || strcmp(s, "ai") == 0)
    return POLICY_BANDIT;

  if (strcmp(s, "baseline") == 0 || strcmp(s, "scan") == 0)
    return POLICY_BASELINE;

  return def_val;
}

/* Blocking IPv4 connect. Mirrors normal telnet behavior on 127.0.0.1 */
...
static int tcp_send_line(const char* host, int port, const char* line, int io_timeout_ms, int retries)
{
  ...
  for (int attempt = 1; attempt <= retries && !stop_requested; ++attempt) {
    ...
    while (!stop_requested) {
      ssize_t rc = recv(fd, drain, sizeof(drain), 0);
      if (rc > 0)
        continue;
      if (rc == 0)
        break;
...
static void bandit_update(int beam_id, double reward)
{
  if (beam_id < 0 || beam_id >= MAX_BEAMS)
    return;

  g_n[beam_id] += 1;
  double n = (double)g_n[beam_id];
  g_q[beam_id] += (reward - g_q[beam_id]) / n;
}

static int bandit_choose_eps_greedy(const int* beams, int n, double epsilon)
{
  if (n <= 0)
    return -1;

  /* Force initial exploration of unseen beams */
  for (int i = 0; i < n; ++i) {
    int b = beams[i];
    if (b >= 0 && b < MAX_BEAMS && g_n[b] == 0)
      return b;
  }
...
int main(int argc, char *argv[])
{
  ...
  beam_policy_t policy = env_policy("BEAM_POLICY", POLICY_BANDIT);
  double epsilon = env_double("BEAM_EPSILON", 0.15);
  int fullscan_every = env_int("BEAM_FULLSCAN_EVERY", 20);
  ...
  if (fullscan_every < 1) fullscan_every = 1;
  ...
  fprintf(stderr, "[xApp] policy=%s epsilon=%.3f fullscan_every=%d scan_mode=%s count=%d base=%d\n",
          policy == POLICY_BANDIT ? "bandit" : "baseline",
          epsilon, fullscan_every,
          use_beam_map ? "map" : "full",
          beam_count_cfg, beam_id_base);
  ...
  while (!stop_requested) {
    ...
    bool do_full_scan = (policy == POLICY_BASELINE) ||
                        (policy == POLICY_BANDIT && (cycle % (uint64_t)fullscan_every == 0));

    if (do_full_scan) {
      fprintf(stderr, "\n[scan] full scan %d beam(s), map=0x%llx\n",
              scan_n, (unsigned long long)obs_bmap);
      ...
      for (int i = 0; i < scan_n && !stop_requested; ++i) {
        ...
        if (policy == POLICY_BANDIT)
          bandit_update(b, r.avg_gain_db);
      ...
      int chosen = bandit_choose_eps_greedy(beams_to_scan, scan_n, epsilon);
      if (chosen < 0) {
        wait_ms_interruptible(500);
        continue;
      }

      fprintf(stderr, "\n[bandit] cycle=%" PRIu64 " choose beam=%d Q=%.3f N=%" PRIu64 " map=0x%llx\n",
              cycle, chosen, g_q[chosen], g_n[chosen], (unsigned long long)obs_bmap);

      probe_result_t r = probe_beam(ctrl_host, ctrl_port, chosen, settle_ms, dwell_ms,
                                    min_samples, io_timeout_ms, ctrl_retries);

      if (r.valid) {
        bandit_update(chosen, r.avg_gain_db);
        fprintf(stderr, "[bandit] beam %d reward=%.2f -> Q=%.3f N=%" PRIu64 "\n",
                chosen, r.avg_gain_db, g_q[chosen], g_n[chosen]);
        fprintf(stderr, "[decision] selected beam %d\n", chosen);
      } else {
        fprintf(stderr, "[bandit] beam %d -> no fresh samples (last_active=%d map=0x%llx)\n",
                chosen, r.last_active_beam, (unsigned long long)r.last_beam_map);
      }
    }
